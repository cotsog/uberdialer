...
// Workable tab click event
$("#workable_type_search").click(function () {
    $('#non_workable_type_search').removeClass('active');
    $("#callback_type_search").removeClass('active');
    $("#workable_type_search").addClass('active');
    $('#gird2 tbody').remove();
    $('#gird3 tbody').remove();
     
    $.ajax({
        type: 'POST',
        url:"/dialer/contacts/getContacts/"+campaign_id+"/1/1/0/"+listid,
        dataType: "json",
        ...
        EditData(newGridName);
        displayGridToolbar('workable');
        nwFilterRules = $.parseJSON(response.filterRules)
        populateFilterRules( nwFilterRules );
...
// Non-workable tab click event
$("#non_workable_type_search").click(function () {
    $('#workable_type_search').removeClass('active');
    $("#non_workable_type_search").addClass('active');
    $("#callback_type_search").removeClass('active');
    $('#list tbody').remove();
    $('#gird3 tbody').remove();
    $.ajax({
        type: 'POST',
        url:"/dialer/contacts/get_non_workable_contacts/"+campaign_id+"/1/1/"+listid,
        dataType: "json",
        beforeSend: function( xhr ) {
            $(".loading").css({"display":"block"});
        }
        ...
        callGrid(griddata, newGrid, newGridName, newPager);
        EditData(newGridName);
        displayGridToolbar('non_workable');
        //get filter rules for non workable
        nwFilterRules = $.parseJSON(response.filterRules)
        populateFilterRules( nwFilterRules );
        $(".loading").css({"display":"none"});
    }).error(function(response) {
...
//Callback tab click event
$("#callback_type_search").click(function () {
    $('#workable_type_search').removeClass('active');
    $("#non_workable_type_search").removeClass('active');
    $("#callback_type_search").addClass('active');
    $('#list tbody').remove();
    $('#gird2 tbody').remove();
    $.ajax({
        type: 'POST',
        url:"/dialer/contacts/getContactByDisposition/"+campaign_id+"/1/1/"+listid+"/2",
        dataType: "json",
        beforeSend: function( xhr ) {
            $(".loading").css({"display":"block"});
        }
        ...
        $("#gbox_list").remove();
        $("#dvGqgrid").append(' <table id="gird3" class="jqglabel"></table> <div id="pager" class="jqgrid-footer"></div>  ');
        var griddata = response;
        var newGrid = jQuery("#gird3");
        newGridName = '#gird3';
        newPager = 'grid3';
        callGrid(griddata, newGrid, newGridName, newPager);
        EditData(newGridName);
        displayGridToolbar('callback');
        //get filter rules for non workable
        nwFilterRules = $.parseJSON(response.filterRules)
        populateFilterRules( nwFilterRules );
        $(".loading").css({"display":"none"});
...
function callGrid(griddata, theGrid, GridName, PagerName) {
	...
	categoriesStr[''] = ""; 
        var callback_display = true;
    }else if(GridName != undefined && GridName == '#gird3'){
        var categoriesStr = {"2":"Callback"};
        var callback_display = false;
    }
    ...
    if(GridName != undefined && GridName == '#list'){
        ...
        new_url = '/dialer/contacts/getContactByDisposition/'+campaign_id+'/1/0/'+listid+"/2";
        row_num = 20;
    }
    ...
    theGrid.jqGrid({
        ...
        loadComplete: function () {
            ...
            if(GridName != undefined && GridName == '#list'){
                ...
                new_url_non_ajax = '/dialer/contacts/get_non_workable_contacts/'+campaign_id+'/1/0/'+listid;
            }
            else if(GridName != undefined && GridName == '#gird3'){
                new_url_non_ajax = '/dialer/contacts/getContactByDisposition/'+campaign_id+'/1/0/'+listid+"/2";
            }
