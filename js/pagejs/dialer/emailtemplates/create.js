...
$('#resource_id').focus(function () {
    ...
    if(resource !="")
    {
        ...
        var postData = 'array='+encodeURIComponent(JSON.stringify(result[0]));
...
//   debug: true,
    rules: {
        ...
        body: {
            required: function() 
            {
                CKEDITOR.instances.body.updateElement(); // update textarea
                //var editorcontent = body.value.replace(/<[^>]*>/gi, ''); // strip tags
