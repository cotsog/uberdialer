...
(function ($) {
		...
		return this.each(function () {
			...
			for (iCol = 0; iCol < p.colNames.length; iCol++) {
				...
				if (!cmi.width) {
					...
					cmi.width = parseInt(cmi.width, 10);
				}
			...
			if (p.multiselect) {
				...
				$(p.cb, ts).bind("click", function () {
					...
					var froz = p.frozenColumns === true ? p.id + "_frozen" : "",
						highlightClass = getGuiStyles("states.select");
					if (this.checked) {
							...
							if (i > 0) {
								if (!$(this).hasClass("ui-subgrid") &&
									...
									if (froz) {
										...
										$("#" + jqID(this.id), ts.grid.fbDiv).addClass(highlightClass);
									}
								}
							}
						});
						...
						$(ts.rows).each(function (i) {
							if (i > 0) {
								if (!$(this).hasClass("ui-subgrid") &&
									...
									$(this).removeClass(highlightClass).attr("aria-selected", "false");
									emp.push(this.id);
									if (froz) {
										$("#jqg_" + jqID(p.id) + "_" + jqID(this.id), ts.grid.fbDiv)[p.propOrAttr]("checked", false);
										$("#" + jqID(this.id), ts.grid.fbDiv).removeClass(highlightClass);
									}
								}
							}
						});
			...
			$(ts).addClass(getGuiStyles("grid", "ui-jqgrid-btable")).append(firstr);
			firstr = null;
			...
			var visibleGridIcon = getIcon("gridMinimize.visible"), hiddenGridIcon = getIcon("gridMinimize.hidden"), showhide = getDef("showhide"),
				arf = p.hidegrid === true ? $("<a role='link' class='ui-jqgrid-titlebar-close ui-corner-all'" + (showhide ? " title='" + showhide + "'" : "") + "/>")
					.hover(
	...
	jgrid.extend({
		...
		showHideCol: function (colname, show) {
			return this.each(function () {
				var $t = this, fndh = false, p = $t.p, brd = jgrid.cell_width ? 0 : p.cellLayout, cw;
				if (!$t.grid) { return; }
		...
		clearGridData: function (clearfooter) {
			return this.each(function () {
				...
				if (p.deepempty) {
					$("#" + gridIdEscaped + " tbody:first tr:gt(0)").remove();
				} else {
		...
		bindKeys: function (settings) {
			...
			return this.each(function () {
				...
				$($t).keydown(function (event) {
					var target = $($t).find("tr[tabindex=0]")[0], id, r, mind, expanded = p.treeReader.expanded_field;
			...
			return this.each(function () {
				...
				$.each(colModel, function (ci) {
					var cm = this, soptions, surl, self, select = "", sot, so, i, searchoptions = cm.searchoptions, editoptions = cm.editoptions,
						...
						stbl = $("<table class='ui-search-table'" + (jgrid.msie && jgrid.msiever() < 8 ? " cellspacing='0'" : "") + "><tr><td class='ui-search-oper'></td><td class='ui-search-input styled custom-search'></td><td class='ui-search-clear' style='display:none;'></td></tr></table>");
