...
class Users_model extends CI_Model {
    ...
    function get_user_filtered_list($limit = "", $offset = "") {
        ...
        $sql = 'SELECT id, telemarketing_offices, group_concat(DISTINCT module) as module, email,user_type,
                `status`,created_at,concat(first_name," ",last_name) as full_name
                ,CASE user_type
                    when "agent" then (SELECT concat(first_name," ",last_name) as `team_leader`
                                        FROM users u where u.id = users.parent_id)
                    else null
                END as parent
                FROM ' . $this->table . ' ';
        $sql .= ' WHERE 1=1' ;
        ...
        if (!in_array($loggedInUserType, $upperManagement)) {
            ...
            if(!empty($subOffices)){
                foreach ($subOffices as $subTmOffice) {
                    $subOfficeFilter .= " OR telemarketing_offices = '" . $subTmOffice . "'";
                }
            }
            $sql .= " AND (telemarketing_offices = '" . $logged_tm_office . "' {$subOfficeFilter} )";
            if(sizeof($this->session->userdata('module')) < 2)
                $sql .= " AND FIND_IN_SET($csv_module_value,module) "; // IN ($csv_module_value)
        }
        $sql .= " group by id ORDER BY full_name asc, created_at desc";
        if ($limit != "" && $offset != "")
            $sql .= " LIMIT ? OFFSET ?";
    ...
    function get_team_leads($logged_tm_office="",$csv_module_value="",$count_array_module_value="",$editUser = false){
        ...
        if(!empty($logged_tm_office)){
            ...
            $subOfficeWhere = "";
            if(!empty($sub_telemarketing_offices) && !$editUser){
                foreach ($sub_telemarketing_offices as $sub_telemarketing_office) {
                    $subOfficeWhere .= " OR telemarketing_offices = '" . $sub_telemarketing_office . "'";
                }
            }
            $sql .= " AND (telemarketing_offices = '".$logged_tm_office."' {$subOfficeWhere})";
        }
    ...
    {
        $this->db->where('telemarketing_offices', $tm_office);
    ...
    public function getUsersByIds($ids, $select = '*')
    {
        $sql = "select {$select} from {$this->table} where id IN ({$ids}) ";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    ...
    function get_by_site($tm_site,$user_type = '') {
        $where = " ( telemarketing_offices  ='".$tm_site."'";
        if ($user_type == "manager") {
            $sub_telemarketing_offices = $this->session->userdata('sub_telemarketing_offices');
            foreach ($sub_telemarketing_offices as $sub_telemarketing_office) {
                $where .= " OR telemarketing_offices = '" . $sub_telemarketing_office . "'";
            }
        }
        $where .= ") ";
        $sql = "SELECT id,CONCAT(first_name,' ',last_name) AS `name`,user_type FROM users WHERE " . $where;
        if($user_type == '') {
            $sql .= " AND user_type IN ('manager','team_leader')";
        } elseif($user_type == 'manager') {
            $sql .= " AND user_type IN ('team_leader')";
        }
        $sql .= " and module in ('tm','tm,appt') ";
    ...
    function getAgents()
    {
        ...
        $sql .=" GROUP BY a.id, a.first_name, a.last_name ORDER BY a.first_name";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }

    function getAgentsByOffice($tmOffice, $additionalFilter = ""){
        $this->db->select("id,CONCAT(first_name,' ',last_name) AS agent_name");
        $this->db->from('users');
        $this->db->where('telemarketing_offices', !empty($tmOffice) ? $tmOffice : NULL);
        $filter = "user_type = 'agent' and status = 'Active' {$additionalFilter}";
