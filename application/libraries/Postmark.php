...
class Postmark {
    ...
    private $data = array();
    var $BATCH_ENDPOINT = 'http://api.postmarkapp.com/email/batch';
    private $batch = array();
    private $info = array();
    ...
    function send($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null) {
        ...
        if (intval($httpCode / 100) != 2) {
			...
			if($this->_subject == 'Contact Form Submission'){
				$_error =& load_class('Exceptions', 'core');
				$this->to('john.wallace@pureincubation.com', 'Enterprise Guide');
				$this->tag('Contact');
				$this->subject('Support Mail Bounce');
				$this->message_html($_error->show_error('An Error Was Encountered', 'Error while mailing. Postmark returned HTTP code ' . $httpCode . ' with message "' . $message . '"', 'error_general', 500));
				$this->send();
				$this->to('stefanie.beecoff@pureincubation.com', 'Enterprise Guide');
				$this->send();
    }
            return array('status'=>0,'error_msg'=>'Error while mailing. Postmark returned HTTP code ' . $httpCode . ' with message "' . $message . '"');
    ...
    //POSTMARK BATCH SEND FUNCTIONS STARTS HERE
    public function batchto($to) {
        $this->data['To'] = $to;
        return $this;
    }

    public function batchsubject($subject) {
        $this->data['subject'] = $subject;
        return $this;
    }

    private function batchplain($msg) {
        $this->data['TextBody'] = $msg;
        return $this;
    }

    private function batchhtml($msg) {
        $this->data['HtmlBody'] = $msg;
        return $this;
    }

    public function batchmessage($msg, $type = 'html') {
        if ($type == 'html') {
            return $this->batchhtml($msg);
        } else {
            return $this->batchplain($msg);
        }
    }
    ...
    public function batchbcc() {
        $this->data['Bcc'] = 'bhavesh.parmar@indusa.com';
        return $this;
    }
    ...
    public function batchsend() {
        if (empty($this->batch)) {
            ...
            $resp = $this->batchmessagessend($this->BATCH_ENDPOINT, $this->batch);
            $this->batch = array();
            return $resp;
        }
    }

    protected function batchmessagessend($endpoint, $data) {
    ...
    //POSTMARK BATCH FUNCTIONS ENDS HERE
    /**
     * Mime Types
     *
     * @access  private
